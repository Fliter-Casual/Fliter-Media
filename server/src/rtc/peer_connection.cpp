...
// 修正 Answer SDP 中的 DTLS 协商方向属性，确保它与 Offer 端匹配，从而保证 DTLS
...
std::string PeerConnectionManager::HandleOffer(const std::string &offer_sdp) {
  ...
  // ========== 4. 设置 onLocalDescription 回调 ==========(当本地 SDP
  // 描述（Local Description）准备好/发生变化时，触发这个回调)
  ...
  // ========== 5. 设置 onLocalCandidate 回调 ==========(当本地 ICE
  // 候选者（Local ICE Candidate）被收集到时，触发这个回调)
  ...
  // ========== 6. 设置 onGatheringStateChange 回调 ====(当 ICE
  // 候选者的收集状态（Gathering State）发生变化时，触发这个回调)
  ...
  pc_->onStateChange([this](::rtc::PeerConnection::State state) {
    std::cout << "RTC PeerConnection State: " << PcStateToString(state)
              << std::endl;
    if (pc_state_cb_) {
  ...
  if (config_.answer_wait_ice_ms > 0) {
    ...
    std::this_thread::sleep_for(
        std::chrono::milliseconds(config_.answer_wait_ice_ms));
...
bool PeerConnectionManager::AddIceCandidate(const std::string& peer_id, const std::string &candidate,
  ...
  if (pc_) {
    try {
      ...
      std::cerr << "[RTC] Failed to add remote candidate: " << e.what()
                << std::endl;
    }
  } else {
    // PC 还未创建，缓冲候选
...
void PeerConnectionManager::Close() {
  if (pc_) {
    pc_->close();
    pc_.reset();
    std::cout << "[RTC] PeerConnection closed." << std::endl;
  }
}

void PeerConnectionManager::SetPcStateCallback(PcStateCallback cb) {
  pc_state_cb_ = std::move(cb);
}

void PeerConnectionManager::SetLocalCandidateCallback(
    LocalCandidateCallback cb) {
  local_cand_cb_ = std::move(cb);
}

void PeerConnectionManager::SetGatheringStateCallback(
    GatheringStateCallback cb) {
